...
const MovieInfo = (props) => {
    ...
    return (
        <>
            <div className="container w-88 flex flex-col lg:flex-col col-white-500 px-8 col-grey-100 text-white">
                        ...
                        <div className=" flex items-center">
                            ...
                            <h3 className="font-medium text-2xl">{parseFloat(props.r && props.r.toFixed(1))}/10</h3>
                        ...
                        <div className="flex text-black text-lg italic">
                            {props.gen && props.gen.map(genre => (
                                <div key={genre.id} className="bg-gray-500 font-medium px-2 my-2 mx-2 rounded-sm max-w-max">
                                &bull; {genre.name}
                                </div>
                            ))}  
                        </div>
                        <div className="flex text-black text-lg font-medium italic mb-2">
                            <div className='bg-green-300 px-3 my-2 mx-2 rounded-sm max-w-max'>&bull; {props.s}</div>
                            {props.sl && props.sl.map(lan => (
                            <div key={lan.iso_639_1} className='bg-orange-400 px-3 my-2 mx-2 rounded-sm max-w-max'>&bull; {lan.english_name}</div>
                            ))}    
                        </div>
                        <div className="flex">
                            <button
